...
namespace RegistryExpert
{
    ...
    public class RegistryInfoExtractor
    {
        ...
        /// <summary>
        /// Clear all caches (call when loading a new hive)
        /// </summary>
        ...
        /// <summary>
        /// Extract system information from SYSTEM/SOFTWARE hives
        /// </summary>
        public string ExtractSystemInfo()
        {
            ...
            sb.AppendLine("═══════════════════════════════════════════════════════════════");
            sb.AppendLine();
            sb.AppendLine($"Hive Type: {_parser.CurrentHiveType}");
            sb.AppendLine();

            bool foundData = false;

            // Computer Name (from SYSTEM hive)
            var computerName = GetValue(@"ControlSet001\Control\ComputerName\ComputerName", "ComputerName");
            if (!string.IsNullOrEmpty(computerName))
            {
                sb.AppendLine($"Computer Name: {computerName}");
                foundData = true;
            }

            // Try alternate paths for computer name
            if (string.IsNullOrEmpty(computerName))
            {
                computerName = GetValue(@"ControlSet001\Control\ComputerName\ActiveComputerName", "ComputerName");
                if (!string.IsNullOrEmpty(computerName))
                {
                    sb.AppendLine($"Computer Name: {computerName}");
                    foundData = true;
            ...
            // Timezone
            var timezone = GetValue(@"ControlSet001\Control\TimeZoneInformation", "TimeZoneKeyName");
            if (string.IsNullOrEmpty(timezone))
                timezone = GetValue(@"ControlSet001\Control\TimeZoneInformation", "StandardName");
            if (!string.IsNullOrEmpty(timezone))
            {
            ...
            // Last Shutdown Time
            var shutdownTime = GetBinaryValue(@"ControlSet001\Control\Windows", "ShutdownTime");
            if (shutdownTime != null && shutdownTime.Length >= 8)
            {
                try
                {
                    ...
                    if (ticks > 0)
                    {
                        ...
                        sb.AppendLine($"Last Shutdown: {dt:yyyy-MM-dd HH:mm:ss}");
                        foundData = true;
                    }
                }
                catch { }
            ...
            // OS Information (from SOFTWARE hive)
            var productName = GetValue(@"Microsoft\Windows NT\CurrentVersion", "ProductName");
            var buildNumber = GetValue(@"Microsoft\Windows NT\CurrentVersion", "CurrentBuild");
            var buildLab = GetValue(@"Microsoft\Windows NT\CurrentVersion", "BuildLab");
            var ubr = GetValue(@"Microsoft\Windows NT\CurrentVersion", "UBR");
            var displayVersion = GetValue(@"Microsoft\Windows NT\CurrentVersion", "DisplayVersion");
            var releaseId = GetValue(@"Microsoft\Windows NT\CurrentVersion", "ReleaseId");
            var editionId = GetValue(@"Microsoft\Windows NT\CurrentVersion", "EditionID");
            var installDate = GetValue(@"Microsoft\Windows NT\CurrentVersion", "InstallDate");
            var registeredOwner = GetValue(@"Microsoft\Windows NT\CurrentVersion", "RegisteredOwner");
            var registeredOrg = GetValue(@"Microsoft\Windows NT\CurrentVersion", "RegisteredOrganization");
            var productId = GetValue(@"Microsoft\Windows NT\CurrentVersion", "ProductId");
            ...
            {
                sb.AppendLine($"Registered Owner: {registeredOwner}");
                foundData = true;
            }
            if (!string.IsNullOrEmpty(registeredOrg))
            {
                sb.AppendLine($"Registered Organization: {registeredOrg}");
                foundData = true;
            }
            ...
            var processor = GetValue(@"HARDWARE\DESCRIPTION\System\CentralProcessor\0", "ProcessorNameString");
            ...
            var systemBiosVersion = GetValue(@"HARDWARE\DESCRIPTION\System\BIOS", "SystemProductName");
            if (!string.IsNullOrEmpty(systemBiosVersion))
            {
                sb.AppendLine($"System: {systemBiosVersion}");
                foundData = true;
            }
            
            var biosVendor = GetValue(@"HARDWARE\DESCRIPTION\System\BIOS", "BIOSVendor");
            var biosVersion = GetValue(@"HARDWARE\DESCRIPTION\System\BIOS", "BIOSVersion");
            if (!string.IsNullOrEmpty(biosVendor))
            {
                sb.AppendLine($"BIOS: {biosVendor} {biosVersion}");
                foundData = true;
            }

            // List available Control Sets (SYSTEM hive)
            var controlSets = new[] { "ControlSet001", "ControlSet002", "ControlSet003" };
            var foundControlSets = new List<string>();
            foreach (var cs in controlSets)
            {
                var key = _parser.GetKey(cs);
                if (key != null)
                    foundControlSets.Add(cs);
            }
            if (foundControlSets.Count > 0)
            {
                sb.AppendLine();
                sb.AppendLine("─── Control Sets ───");
                foreach (var cs in foundControlSets)
                {
                    sb.AppendLine($"  • {cs}");
                }
                foundData = true;

                // Try to get current control set
                ...
                if (selectKey != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(current))
                        sb.AppendLine($"  Current: ControlSet00{current}");
                }
            }

            // If this is a SYSTEM hive, show services count
            var servicesKey = _parser.GetKey(@"ControlSet001\Services");
            if (servicesKey?.SubKeys != null)
            {
                sb.AppendLine();
                sb.AppendLine("─── Services Summary ───");
                sb.AppendLine($"  Total Services: {servicesKey.SubKeys.Count}");
                
                var drivers = servicesKey.SubKeys.Where(k => 
                {
                    var type = k.Values.FirstOrDefault(v => v.ValueName == "Type")?.ValueData?.ToString();
                    return type == "1" || type == "2"; // Kernel & FS drivers
                }).Count();
                sb.AppendLine($"  Drivers: {drivers}");
                foundData = true;
            }
            ...
            {
                sb.AppendLine();
                sb.AppendLine("No system information found in this hive.");
                sb.AppendLine("This is expected if the loaded hive is not a SYSTEM or SOFTWARE hive.");
                sb.AppendLine();
                sb.AppendLine("For system information, please load:");
                sb.AppendLine("  • SYSTEM hive (C:\\Windows\\System32\\config\\SYSTEM)");
                sb.AppendLine("  • SOFTWARE hive (C:\\Windows\\System32\\config\\SOFTWARE)");
            }
        ...
        /// <summary>
        /// Extract user information from SAM/NTUSER hives
        /// </summary>
        public string ExtractUserInfo()
        {
            ...
            // From SAM hive - Local Users
            var usersKey = _parser.GetKey(@"SAM\Domains\Account\Users\Names");
            if (usersKey?.SubKeys != null)
            {
                sb.AppendLine("─── Local User Accounts ───");
                foreach (var userKey in usersKey.SubKeys.OrderBy(k => k.KeyName))
                {
                    sb.AppendLine($"  • {userKey.KeyName}");
                }
                sb.AppendLine();
            }

            // From SOFTWARE - ProfileList
            var profilesKey = _parser.GetKey(@"Microsoft\Windows NT\CurrentVersion\ProfileList");
            if (profilesKey?.SubKeys != null)
            {
                ...
                foreach (var profile in profilesKey.SubKeys)
                {
                    ...
                    {
                        sb.AppendLine($"  SID: {profile.KeyName}");
                        sb.AppendLine($"      Path: {profilePath}");
                    }
                }
                sb.AppendLine();
            }

            // From NTUSER.DAT - User specific info
            sb.AppendLine("─── User Settings (NTUSER.DAT) ───");
            
            // Recent documents
            var recentDocs = _parser.GetKey(@"Software\Microsoft\Windows\CurrentVersion\Explorer\RecentDocs");
            if (recentDocs != null)
            {
                sb.AppendLine("  Recent Documents: Found");
                sb.AppendLine($"    Extensions with recent files: {recentDocs.SubKeys?.Count ?? 0}");
            }

            // TypedPaths (Explorer address bar history)
            var typedPaths = _parser.GetKey(@"Software\Microsoft\Windows\CurrentVersion\Explorer\TypedPaths");
            if (typedPaths != null && typedPaths.Values.Count > 0)
            {
                sb.AppendLine("  Explorer Typed Paths:");
                foreach (var value in typedPaths.Values.Take(10))
                {
                    if (!string.IsNullOrEmpty(value.ValueData?.ToString()))
                        sb.AppendLine($"    • {value.ValueData}");
                }
            }

            // RunMRU (Run dialog history)
            var runMru = _parser.GetKey(@"Software\Microsoft\Windows\CurrentVersion\Explorer\RunMRU");
            if (runMru != null)
            {
                sb.AppendLine("  Run Dialog History:");
                foreach (var value in runMru.Values.Where(v => v.ValueName != "MRUList").Take(10))
                {
            ...
            var userAssist = _parser.GetKey(@"Software\Microsoft\Windows\CurrentVersion\Explorer\UserAssist");
            if (userAssist?.SubKeys != null && userAssist.SubKeys.Count > 0)
            {
                sb.AppendLine($"  UserAssist: {userAssist.SubKeys.Count} GUIDs tracked");
            }
        ...
        /// <summary>
        /// Extract installed software information from SOFTWARE hive
        /// </summary>
        public string ExtractSoftwareInfo()
        {
            var sb = new StringBuilder();
            sb.AppendLine("═══════════════════════════════════════════════════════════════");
            sb.AppendLine("                    SOFTWARE INFORMATION");
            sb.AppendLine("═══════════════════════════════════════════════════════════════");
            sb.AppendLine();

            var installedPrograms = new List<(string Name, string Version, string Publisher, string InstallDate)>();

            // 64-bit programs
            var uninstallKey = _parser.GetKey(@"Microsoft\Windows\CurrentVersion\Uninstall");
            if (uninstallKey?.SubKeys != null)
            {
                foreach (var program in uninstallKey.SubKeys)
                {
                    ...
                    if (!string.IsNullOrEmpty(displayName))
                    {
                        ...
                        installedPrograms.Add((displayName, version, publisher, installDate));
                    }
                }
            }

            // 32-bit programs on 64-bit Windows
            var uninstall32Key = _parser.GetKey(@"WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall");
            if (uninstall32Key?.SubKeys != null)
            {
                foreach (var program in uninstall32Key.SubKeys)
                {
                    var displayName = program.Values.FirstOrDefault(v => v.ValueName == "DisplayName")?.ValueData?.ToString();
                    if (!string.IsNullOrEmpty(displayName) && !installedPrograms.Any(p => p.Name == displayName))
                    {
                        var version = program.Values.FirstOrDefault(v => v.ValueName == "DisplayVersion")?.ValueData?.ToString() ?? "";
                        var publisher = program.Values.FirstOrDefault(v => v.ValueName == "Publisher")?.ValueData?.ToString() ?? "";
                        var installDate = program.Values.FirstOrDefault(v => v.ValueName == "InstallDate")?.ValueData?.ToString() ?? "";
            ...
            sb.AppendLine($"─── Installed Programs ({installedPrograms.Count}) ───");
            sb.AppendLine();
            
            foreach (var (name, version, publisher, installDate) in installedPrograms.OrderBy(p => p.Name))
            {
                sb.AppendLine($"  {name}");
                ...
                if (!string.IsNullOrEmpty(installDate))
                    sb.AppendLine($"    Installed: {FormatInstallDate(installDate)}");
                sb.AppendLine();
            }
            ...
            var runKey = _parser.GetKey(@"Microsoft\Windows\CurrentVersion\Run");
            if (runKey != null)
            {
                sb.AppendLine("  HKLM\\...\\Run:");
                foreach (var value in runKey.Values)
                {
                    sb.AppendLine($"    • {value.ValueName}: {value.ValueData}");
                }
            }

            var runOnceKey = _parser.GetKey(@"Microsoft\Windows\CurrentVersion\RunOnce");
            if (runOnceKey != null && runOnceKey.Values.Count > 0)
            {
                sb.AppendLine("  HKLM\\...\\RunOnce:");
                foreach (var value in runOnceKey.Values)
                {
                    sb.AppendLine($"    • {value.ValueName}: {value.ValueData}");
                }
            }
            ...
            // From NTUSER.DAT
            var userRunKey = _parser.GetKey(@"Software\Microsoft\Windows\CurrentVersion\Run");
            if (userRunKey != null && userRunKey.Values.Count > 0)
            {
                sb.AppendLine("  HKCU\\...\\Run:");
                foreach (var value in userRunKey.Values)
                {
                    sb.AppendLine($"    • {value.ValueName}: {value.ValueData}");
                }
            }
            ...
            if (servicesKey?.SubKeys != null)
            {
                var services = servicesKey.SubKeys
                    .Where(s => s.Values.Any(v => v.ValueName == "ImagePath"))
                    .Take(50)
                    .ToList();
                
                sb.AppendLine($"  Total Services: {servicesKey.SubKeys.Count}");
                sb.AppendLine("  Sample Services (first 50):");
                
                foreach (var service in services)
                {
                    var imagePath = service.Values.FirstOrDefault(v => v.ValueName == "ImagePath")?.ValueData?.ToString() ?? "";
                    var startType = service.Values.FirstOrDefault(v => v.ValueName == "Start")?.ValueData?.ToString() ?? "";
                    var displayName = service.Values.FirstOrDefault(v => v.ValueName == "DisplayName")?.ValueData?.ToString() ?? service.KeyName;
                    
                    sb.AppendLine($"    • {displayName}");
                    sb.AppendLine($"      Start: {GetStartTypeName(startType)} | Path: {TruncatePath(imagePath, 60)}");
                }
            }
        ...
        /// </summary>
        public string ExtractNetworkInfo()
        {
            var sb = new StringBuilder();
            sb.AppendLine("═══════════════════════════════════════════════════════════════");
            ...
            if (interfacesKey?.SubKeys != null)
            {
                foreach (var iface in interfacesKey.SubKeys)
                {
                    ...
                    var subnet = iface.Values.FirstOrDefault(v => v.ValueName == "DhcpSubnetMask")?.ValueData?.ToString()
                    ...
                    {
                        sb.AppendLine($"  Interface: {iface.KeyName.Substring(0, Math.Min(36, iface.KeyName.Length))}...");
                        sb.AppendLine($"    DHCP: {(dhcpEnabled == "1" ? "Enabled" : "Disabled")}");
            ...
            // Network Profiles (previously connected networks)
            sb.AppendLine("─── Network Profiles (Known Networks) ───");
            var networkListKey = _parser.GetKey(@"Microsoft\Windows NT\CurrentVersion\NetworkList\Profiles");
            if (networkListKey?.SubKeys != null)
            {
                foreach (var profile in networkListKey.SubKeys)
                {
                    var profileName = profile.Values.FirstOrDefault(v => v.ValueName == "ProfileName")?.ValueData?.ToString();
                    ...
                    var dateCreated = profile.Values.FirstOrDefault(v => v.ValueName == "DateCreated")?.ValueDataRaw;
                    var dateLastConnected = profile.Values.FirstOrDefault(v => v.ValueName == "DateLastConnected")?.ValueDataRaw;

                    if (!string.IsNullOrEmpty(profileName))
                    {
                        sb.AppendLine($"  • {profileName}");
                        if (!string.IsNullOrEmpty(description) && description != profileName)
                            sb.AppendLine($"    Description: {description}");
                        if (!string.IsNullOrEmpty(category))
                            sb.AppendLine($"    Category: {GetNetworkCategory(category)}");
                        if (dateCreated != null && dateCreated.Length >= 8)
                            sb.AppendLine($"    First Connected: {ParseNetworkDate(dateCreated)}");
                        if (dateLastConnected != null && dateLastConnected.Length >= 8)
                            sb.AppendLine($"    Last Connected: {ParseNetworkDate(dateLastConnected)}");
                    }
                }
            }

            // Wireless networks from NTUSER or SOFTWARE
            sb.AppendLine();
            sb.AppendLine("─── Wireless Network SSIDs ───");
            var wlanKey = _parser.GetKey(@"Microsoft\Windows NT\CurrentVersion\NetworkList\Signatures\Unmanaged");
            if (wlanKey?.SubKeys != null)
            {
                foreach (var sig in wlanKey.SubKeys)
                {
                    var ssid = sig.Values.FirstOrDefault(v => v.ValueName == "FirstNetwork")?.ValueData?.ToString();
            ...
            if (sharesKey != null && sharesKey.Values.Count > 0)
            {
                ...
                {
                    sb.AppendLine($"  • {share.ValueName}");
                }
            }
            else
            {
                sb.AppendLine("  No shares found or not a SYSTEM hive");
            }
        ...
        public string GenerateFullReport()
        {
            ...
            sb.AppendLine("╔═══════════════════════════════════════════════════════════════╗");
            sb.AppendLine("║          REGISTRY EXPERT - FULL ANALYSIS REPORT              ║");
            sb.AppendLine("╚═══════════════════════════════════════════════════════════════╝");
            sb.AppendLine();
            sb.AppendLine($"Report Generated: {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
            sb.AppendLine($"Hive File: {_parser.FilePath}");
            sb.AppendLine($"Hive Type: {_parser.CurrentHiveType}");
            
            var stats = _parser.GetStatistics();
            sb.AppendLine($"File Size: {stats.FormattedFileSize}");
            sb.AppendLine($"Total Keys: {stats.TotalKeys:N0}");
            sb.AppendLine($"Total Values: {stats.TotalValues:N0}");
            sb.AppendLine();
            sb.AppendLine(new string('═', 65));
            sb.AppendLine();

            sb.AppendLine(ExtractSystemInfo());
            sb.AppendLine();
            sb.AppendLine(ExtractUserInfo());
            sb.AppendLine();
            sb.AppendLine(ExtractSoftwareInfo());
            sb.AppendLine();
            sb.AppendLine(ExtractNetworkInfo());
            sb.AppendLine();
            ...
            var usbStorKey = _parser.GetKey(@"ControlSet001\Enum\USBSTOR");
            if (usbStorKey?.SubKeys != null)
            {
                foreach (var deviceClass in usbStorKey.SubKeys)
                {
                    sb.AppendLine($"  Device: {deviceClass.KeyName}");
                    if (deviceClass.SubKeys != null)
                    {
                        foreach (var device in deviceClass.SubKeys)
                        {
                            var friendlyName = device.Values.FirstOrDefault(v => v.ValueName == "FriendlyName")?.ValueData?.ToString();
                            sb.AppendLine($"    Serial: {device.KeyName}");
            ...
            {
                sb.AppendLine("  No USB storage devices found or not a SYSTEM hive");
            }

            // MRU Lists
            sb.AppendLine();
            sb.AppendLine("═══════════════════════════════════════════════════════════════");
            sb.AppendLine("                        MRU LISTS");
            sb.AppendLine("═══════════════════════════════════════════════════════════════");
            sb.AppendLine();
            
            // OpenSavePidlMRU
            var openSaveMru = _parser.GetKey(@"Software\Microsoft\Windows\CurrentVersion\Explorer\ComDlg32\OpenSavePidlMRU");
            if (openSaveMru?.SubKeys != null)
            {
                sb.AppendLine("─── Open/Save Dialog MRU ───");
                foreach (var ext in openSaveMru.SubKeys.Take(10))
                {
                    sb.AppendLine($"  Extension: {ext.KeyName} ({ext.Values.Count} entries)");
                }
            }

            // LastVisitedPidlMRU
            var lastVisitedMru = _parser.GetKey(@"Software\Microsoft\Windows\CurrentVersion\Explorer\ComDlg32\LastVisitedPidlMRU");
            if (lastVisitedMru != null)
            {
                sb.AppendLine($"  Last Visited Directories: {lastVisitedMru.Values.Count} entries");
            }
        ...
        private string DetermineOverallProtocolStatus(Registry.Abstractions.RegistryKey protocolKey)
        {
        ...
        /// <summary>
        /// Get services information as structured sections (for non-Services category)
        /// </summary>
        public List<AnalysisSection> GetServicesAnalysis()
        {
            // This is now handled specially in the UI with GetAllServices()
            var sections = new List<AnalysisSection>();
            var services = GetAllServices();
            
            if (services.Count > 0)
            {
                var section = new AnalysisSection { Title = $"🔧 All Services ({services.Count})" };
                foreach (var svc in services.OrderByDescending(s => s.StartType == "4").ThenBy(s => s.ServiceName))
                {
                    section.Items.Add(new AnalysisItem
                    {
                        Name = svc.ServiceName,
                        Value = svc.StartTypeName,
                        RegistryPath = svc.RegistryPath,
                        RegistryValue = $"Start = {svc.StartType} ({svc.StartTypeName})" +
                            (!string.IsNullOrEmpty(svc.ImagePath) ? $"\nImagePath = {svc.ImagePath}" : "") +
                            (!string.IsNullOrEmpty(svc.Description) ? $"\nDescription = {svc.Description}" : "")
                    });
                }
            ...
            {
                var noDataSection = new AnalysisSection { Title = "ℹ️ Notice" };
                noDataSection.Items.Add(new AnalysisItem { Name = "Info", Value = "No services information found in this hive type", RegistryPath = "", RegistryValue = "Services are stored in the SYSTEM hive" });
                noDataSection.Items.Add(new AnalysisItem { Name = "Tip", Value = "Load SYSTEM hive for services info", RegistryPath = "", RegistryValue = "Navigate to ControlSet001\\Services" });
                sections.Add(noDataSection);
            }
