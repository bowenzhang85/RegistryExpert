...
public class TransactionLog
{
    ...
    public TransactionLog(byte[] rawBytes, string logFile)
    {
        ...
        {
            Debug.WriteLine("ERROR: Data in byte array is not a Registry transaction log (bad signature)");
    ...
    public TransactionLog(string logFile)
    {
        ...
        if (h.Length > FileBytes.Length)
        {
            ...
            Debug.WriteLine($"Log size exceeds hive size. Adding {diff} bytes to hive bytes");
        ...
        if (!HasValidSignature())
        {
            Debug.WriteLine($"ERROR: {logFile} is not a Registry transaction log (bad signature)");
    ...
    private void Initialize()
    {
        ...
        Debug.WriteLine($"Got transaction log header. Embedded file name {Header.FileName}. Base Name {fNameBase}");
    ...
    public byte[] UpdateHiveBytes(byte[] hiveBytes)
    {
        ...
        foreach (var transactionLogEntry in TransactionLogEntries)
        {
            ...
            {
                Debug.WriteLine($"Skipping transaction log entry with sequence # 0x{transactionLogEntry.SequenceNumber:X}. Hash verification failed");
                continue;
            }
